...
rf_model = joblib.load('FifaPredictionmodel.pkl')
scaler = joblib.load('Scaler.pkl')

# Function to preprocess user input
def preprocess_input(value_eur, release_clause_eur, dob, potential, movement_reactions, age, gk):
    # Converting the date of birth to a numeric value
    dob = float(dob.replace('-', ''))
...
def main():
    ...
    release_clause_eur = st.sidebar.slider('Release Clause (in Euros)', 0.0, 100000000.0, 50.0)
    dob = st.sidebar.text_input('Date of Birth', '1990-01-01')  
    potential = st.sidebar.slider('Potential', 50, 100, 75)
    movement_reactions = st.sidebar.slider('Movement Reactions', 50, 100, 75)
    age = st.sidebar.slider('Age', 18, 50, 20)
    gk = st.sidebar.slider('Goalkeeping', 0, 100, 10)

    if st.sidebar.button('Predict'):
        # Preprocessing the  user input and making predictions
        user_input_scaled = preprocess_input(value_eur, release_clause_eur, dob, potential, movement_reactions, age, gk)
...
#  python -m streamlit run deployM.py (Run in command line to run app)